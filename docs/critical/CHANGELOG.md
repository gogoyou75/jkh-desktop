...
## Проверка расчётного года при импорте Excel

* Добавлено обязательное требование наличия года в A15
* Импорт блокируется, если A15 пустая или некорректная
* Даты оплат проверяются относительно A15
* Допуск ±1 год — предупреждение
* Отличие более 1 года — ошибка
* Добавлен блок анализа перед импортом
* Кнопка подтверждения года не используется
