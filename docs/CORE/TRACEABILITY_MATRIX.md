...
| `POST /api/abonent_summary/mark_dirty` и frontend dirty tracking помечают конкретный UID как dirty без пересчёта | LOGIC_SPEC 21.7.2 | `backend/app.py`, `web/data.js`, `web/payment_table.js`, `web/import_xls.html`, `web/abonent_card.html` | ✅ OK | Dirty означает, что summary нельзя использовать как актуальные totals; ошибки endpoint логируются и не блокируют основное сохранение. |
| Future batch recalc dirty marker for `affected_uids` remains separate from summary dirty endpoint | LOGIC_SPEC 21.8 | future API | ⚪ IDEA | Batch recalculation is not triggered by dirty marking. |
