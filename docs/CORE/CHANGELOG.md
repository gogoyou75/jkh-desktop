...
## 2026-05-20
- Добавлен разделённый режим пересчёта карточки: full и `temporary_court_period`.
- Временный режим считает только `period_report_totals` по выбранному периоду и не выполняет полный расчёт всей истории.
- Исправлен рендер карточки: основной блок totals и временный блок period report отображаются независимо.
- Открытие справки ограничено условиями: активный период + рассчитанный `period_report_totals`.
- Справка не запускает autoaccrual и не модифицирует ledger.
