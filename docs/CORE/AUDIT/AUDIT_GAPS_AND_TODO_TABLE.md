...
*| 2 | CalcEngine / ставки | Если дата расчёта раньше первой доступной ставки, система не должна применять первую ставку задним числом | CODE-FIX | P0 | Остановить расчёт и показать понятное предупреждение пользователю (без fallback) | calc_engine.js / spravka_sud.js / карточка |
...
*| 6 | Payments | Старые CRITICAL-комментарии говорят `payments_<LS>`, но текущий канон — `payments_<uid>` | CODE-FIX | P0 | Обновить CRITICAL-комментарии: явно указать legacy-статус `payments_<LS>` и текущий канон `payments_<uid>`, не меняя рабочую логику | payment_table.js / spravka_sud.js / autoaccrual_engine.js |
