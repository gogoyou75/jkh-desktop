...
| 58 | Debt transfer | `WITH_DEBT` не должен превращать ошибку frozen debt calculation в нулевой долг | DONE | P0 | Перенос долга требует успешный frozen debt calculation; ошибка расчёта/JSON не становится `principal: 0` / `penalty: 0` | LOGIC_SPEC / TRACEABILITY / CHANGELOG; commit Prevent unsafe zero debt transfer fallback |
---
