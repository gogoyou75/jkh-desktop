...
- `summary.totals` фиксируется как полная задолженность на дату (full mode через `calcTotalsAsOfAdjusted`).
- `summary.period_report_totals` — временный расчёт для судебной справки за выбранный период **без входящего остатка**.
- `period_report_totals` не меняет ledger и не является полной задолженностью.
