...
## 12. Судебный временный период (temporary court period)

- `summary.totals` = **полная задолженность на дату** (основной долг, пеня, итог) и не подменяется временным периодом.
- `summary.period_report_totals` = **временный расчёт для судебной справки** за выбранный период `from/to` без входящего остатка (`mode: temporary_court_period_without_opening_balance`).
- `period_report_totals` — производный отчётный блок; он **не меняет ledger**, **не меняет payments_<uid>**, **не является полной задолженностью**.
- Для режима `temporary_court_period` запрещено запускать тяжёлый полный пересчёт всей истории только ради временной справки; допускается расчёт только по строкам выбранного периода.
- Открытие судебной справки разрешено только после явного расчёта периода и при активном `calc_period_active_<uid> = "1"` и валидном `calc_period_<uid>`.
