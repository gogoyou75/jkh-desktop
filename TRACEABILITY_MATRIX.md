...
| Phase 2 Core has no persistence or browser-data reads | LOGIC_SPEC Phase 2 shared Core boundary | web/full_recalc_core.js / web/data.js | OK | The Core consumes explicit rows/as-of pairs and an injected calculator. Browser `JKHCalcEngine` remains the sole formula source. |
| Explicit financial input calculation has no hidden browser reads | LOGIC_SPEC Phase 2B explicit financial inputs | web/calc_engine.js / web/data.js / web/full_recalc_core.test.js | OK | Browser compatibility builds inputs first; explicit totals accept responsibility, rates, exclusions, freeze and transfer without invoking loaders. |
| Node shadow runner is no-write | LOGIC_SPEC Phase 3 no-write shadow comparison | scripts/run_full_recalc_shadow.js / shared/full_recalc_shadow_compare.js | OK | JSON-only CLI has strict diff/hash/exit-code contracts and imports no browser adapter, persistence or network API. |
| Browser Shadow Export loads the canonical compare API | LOGIC_SPEC Phase 3 browser export dependency | web/abonent_card.html / docker-compose.yml / nginx/default.conf / scripts/test_shadow_compare_static_asset_contract.js | OK | The single `shared/full_recalc_shadow_compare.js` source is mounted read-only at Nginx `/shared`; the card path resolves to that static asset. |
