...
(function () {
  ...
  const STORAGE = (APP_CTX && APP_CTX.storage) ? APP_CTX.storage : localStorage;
  ...
  function removeProjectKeys() {
    // exact
    PROJECT_KEY_EXACT.forEach((k) => STORAGE.removeItem(k));

    // prefixes (только ключи активной базы)
    if (APP_CTX && typeof APP_CTX.listBaseKeysForNamespace === "function") {
      const keys = APP_CTX.listBaseKeysForNamespace(APP_CTX.getReadNamespace());
      keys.forEach((k) => {
  ...
  function loadFromStorage() {
    const raw = STORAGE.getItem(KEY_DB);
    const parsed = safeJsonParse(raw, null);
    return parsed && typeof parsed === "object" ? parsed : null;
  }

  function saveToStorage(db) {
    STORAGE.setItem(KEY_DB, JSON.stringify(db));
  }
  ...
  function seedDemoKeys_1006_1008() {
    ...
    // 2) last abonent
    STORAGE.setItem("last_abonent_id", "1008");

    // 3) источники платежей
    STORAGE.setItem("payment_sources_v1", JSON.stringify(["Платёж 1", "Платёж 2", "Платёж 3"]));

    // 4) тарифы (как у тебя на скрине: content/repair)
    STORAGE.setItem("tariffs_content_repair_v1", JSON.stringify({
      content: [{ date: "2025-01-01", rate: 10 }],
      repair: [{ date: "2025-01-01", rate: 10 }]
    }));

    // 5) ставки рефинансирования (normal + moratorium)
    STORAGE.setItem("refinancing_rates_normal_v1", JSON.stringify([
      { from: "01.01.2025", rate: "11" }
    ]));
    STORAGE.setItem("refinancing_rates_moratorium_v1", JSON.stringify([
      { from: "01.04.2025", rate: "5" }
    ]));

    // 6) периоды расчёта (пустые, как на скрине)
    ["1006", "1008"].forEach((id) => {
      STORAGE.setItem("calc_period_" + id, JSON.stringify({ from: "", to: "" }));
      STORAGE.setItem("calc_period_active_" + id, "0");
      STORAGE.setItem("report_period_" + id, JSON.stringify({ from: "", to: "" }));
    });

    // 7) платежи — намеренно как “проверочный кейс”
    //    (формат совместим с index.html, который суммирует accrued/paid и читает paid_date)
    STORAGE.setItem("payments_1006", JSON.stringify([
    ...
    STORAGE.setItem("payments_1008", JSON.stringify([
      { id: 1, year: "2025", month: "01", accrued: 200, paid: 0, paid_date: "", source: "Платёж 1", payment_period: "" }
    ]));
  }
