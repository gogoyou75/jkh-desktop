...
(function () {
  ...
  function isMoratoriumActive(abonentId){ return _storGet(moratoriumKey(abonentId)) === "1"; }

  function loadExcludes(abonentId){
    try{
      const raw = _storGet(excludePeriodsKey(abonentId));
  ...
  function getFreezeToISO(abonentId){
    try{
      const v = String(_storGet(freezeKey(abonentId)) || "").trim();
  ...
  function getTransferBalance(abonentId){
    try{
      ...
      let penalty = 0;
      try{
        const oldRaw = _storGet("payments_" + String(prevId));
  ...
  function buildPaymentEventsFromRows(rows, abonentId){
    ...
    try{
      const active = String(_storGet('calc_period_active_' + id) || '0') === '1';
      if (active){
        const raw = _storGet('calc_period_' + id);
        if (raw){
          const obj = JSON.parse(raw);
